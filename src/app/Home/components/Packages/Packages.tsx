  ...
  return (
    <Box py={{ md: 10, sm: 6, xs: 6 }} bgcolor={COLOR.gray.light}>
      <Container maxWidth="lg">
        ...
        <Grid container spacing={{ md: 6 }} px={{ md: 3 }} textAlign={"center"}>
          {PACKAGES.map((feature, i) => (
            <Grid item md={4} xs={12} mt={5} key={i}>
              <Box bgcolor={"white"} boxShadow={"0 0 12px 1px lightGray"} p={4}>
                <DiamondOutlined
                  sx={{
                    fontSize: "4em",
                ...
                <ButtonComp title="Purchase Packages" width="200px"/>
                </Box>
              </Box>
            </Grid>
          ))}
